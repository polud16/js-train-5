...
function sumArray(array) {
  ...
  return sum; // Ініціалізуємо змінну для зберігання суми
  // Проходимося по всіх індексах масиву
...
function multiplyArray(array) {
  let count = 1; // Ініціалізуємо змінну для зберігання добутку
  for (const num of array) {
    count *= num;
  }
  return count; // Проходимося по всіх значеннях масиву
...
function concatenateStrings(array, subString) {
  console.log(array.toString().includes(subString)); // Використовуємо метод toString() для перетворення масиву в рядок
...
function addTenToEach(array) {
  const resultArray = [];
  for (const index in array) {
    const number = array[index] + 10;
    resultArray.push(number);
  }
  console.log(resultArray); // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату
...
function multiplyByIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const multipliedNumber = arr[i] * i;
